...
// /yarn:    yarn v2
const runFromYarn = true; // /(yarn\/bin|\/yarn$)/.test(process.env.npm_execpath || "");

// const npmUA = process.env.npm_config_user_agent || "";
const isYarnV2 = true; // /\byarn\/([2-9]\d*|1\d+)\./.test(npmUA);
...
export function execSync(command, opts) {
  ...
  if (!runFromPackageScript) {
    env.PATH = prependNodeModulesBinIntoPath(import.meta.dirname, env.PATH);
    env.PATH = prependNodeModulesBinIntoPath(process.cwd(), env.PATH);
  }

  if(!opts || opts.showCmd !== false)
    console.error(`${resetColor}+ (sync) ${bin} ${args.join(" ")}`);
 ...
 * @returns {string[]}
 */
export function resolveCmd(defaultBinName, requirePath) {
  if (requirePath) {
    try {
